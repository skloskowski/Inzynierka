...
\begin{tabular}{l}
$S = s \cdot t = s \cdot N_\mathrm{DIT} \cdot \mathrm{DIT}$ – całkowity sygnał pochodzący od obiektu [\elec] \\
...
$s$ – liczba elektronów zgromadzonych na sekundę od obiektu [\elec] \\ \\
\end{tabular}
...
for each pixel (x, y):
    ...
    Metoda Auto Adaptive Weighted Average jest algorytmem iteracyjnym, która przypisuje każdemu pikselowi wagę zależną od odchylenia od bieżącej średniej. Wagi są wyznaczane za pomocą funkcji adaptacyjnej kontrolowanej parametrami $\alpha$ oraz $\beta$. Metoda pozwala na większa swobodę w implementacji - funkcja adaptacyjna może być zależna nie tylko od wartości pikseli na fotografii ale również od statystycznie obliczonej jakości zdjęcia czy innych wymaganych parametrów. Założenia algorytmu przedstawiono w kodzie \ref{lst:adaptive}.
